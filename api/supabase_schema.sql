...
-- Access monitoring tables
-- Anonymous visitors may insert their own session/events from the static app.
-- Reading access logs is limited to authenticated dashboard/admin users by default.
...
create table if not exists access_events (
  ...
  visitor_id uuid not null,
  event_type text not null check (event_type in ('page_view', 'module_view', 'profile_saved', 'location_enabled', 'location_denied', 'export_csv', 'auth_link_requested', 'auth_signed_in', 'auth_signed_out')),
  module_name text,
  path text,
